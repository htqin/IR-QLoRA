...
class GPTQLoraLinear(torch.nn.Linear, LoraLayer):
    def __init__(
        ...
        **kwargs,
        ...
        self.lora_default_A_scale = torch.nn.Parameter(torch.zeros([1], dtype=torch.float32).to(self.weight.device), requires_grad=True)
        self.lora_default_B_scale = torch.nn.Parameter(torch.zeros([1], dtype=torch.float32).to(self.weight.device), requires_grad=True)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            lora_dropout = self.lora_dropout[self.active_adapter[0]]
            scale = self.scaling[self.active_adapter[0]]

            x = x.type_as(lora_A.weight.data)
            x = lora_A(lora_dropout(self.qa_pool(x) * group_size)) + self.lora_default_A_scale * x.reshape([_ for _ in x.shape[:-1]] + [lora_A.out_features] + [-1]).mean(dim=-1)
            x = (lora_B(x).reshape([_ for _ in x.shape] + [-1]) + self.lora_default_B_scale * x.unsqueeze(-1)).reshape([_ for _ in x.shape[:-1]] + [-1])
            x = x * scale
            adapter_result = x.type_as(result)
...
class GPTQLoraModel(LoraModel):
    def _my_module(self, target):
        ...
        kwargs = {"r": lora_config.r, "lora_alpha": lora_config.lora_alpha, "lora_dropout": lora_config.lora_dropout, "fan_in_fan_out": lora_config.fan_in_fan_out, "init_lora_weights": lora_config.init_lora_weights,}
        bias = False
    ...
    def _replace_module(self, parent_module, child_name, new_module, old_module):
        new_module = self._my_module(old_module)
...
class GPTQSVDLinear(torch.nn.Linear, AdaLoraLayer):
    def __init__(
        ...
        **kwargs,
        ...
        self.update_layer(adapter_name, r, lora_alpha, lora_dropout, init_lora_weights)
        self.active_adapter[0] = adapter_name
        ...
        if self.disable_adapters:
            ...
            result += (
                (
                    ...
                    @ (self.lora_A[self.active_adapter[0]] * self.lora_E[self.active_adapter[0]]).T
                    @ self.lora_B[self.active_adapter[0]].T
                )
                * self.scaling[self.active_adapter[0]]
                / (self.ranknum[self.active_adapter[0]] + 1e-5)
            )
        else:
            result = self.linear_module(x)
        return result
